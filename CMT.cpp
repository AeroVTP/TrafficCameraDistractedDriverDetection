...
void track(cv::Mat im_prev, cv::Mat im_gray, const std::vector<std::pair<cv::KeyPoint, int> >& keypointsIN, std::vector<std::pair<cv::KeyPoint, int> >& keypointsTracked, std::vector<unsigned char>& status, int THR_FB)
{
    ...
    if(keypointsIN.size() > 0)
    {
        ...
        std::cout << pts.size() << " size of points" << std::endl;
        std::cout << nextPts.size() << " size of points" << std::endl;
        nextPts = pts;
        //Calculate forward optical flow for prev_location
        //cv::calcOpticalFlowPyrLK(im_prev, im_gray, pts, nextPts, status, err);
        cv::Size s(21, 21);
        cv::calcOpticalFlowPyrLK(im_prev, im_gray, pts, nextPts, status, err, s, 0, cv::TermCriteria(cv::TermCriteria::COUNT+cv::TermCriteria::EPS, 30, 0.01),  cv::OPTFLOW_USE_INITIAL_FLOW, 0);

        std::cout << pts.size() << "DEATH" << std::endl;
...
void CMT::initialise(cv::Mat im_gray0, cv::Point2f topleft, cv::Point2f bottomright)
{
    ...
    std::cout << background_keypoints.size() << " selected_keypoints" << std::endl;

    drawKeypoints(im_gray0, background_keypoints, im_gray0,  (0, 255, 0), 0);


    //    while(true){
        	imshow("CMT TEST" , im_gray0); imwrite("CMT TEST.TIFF", im_gray0);
    //  drawKeypoints(im_gray0, selected_keypoints, im_gray0,  (0, 255, 0), 0);

      /*
      while(true)
      	imshow("CMT TEST" , im_gray0);
*/
